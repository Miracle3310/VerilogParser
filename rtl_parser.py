...
class rtl_parser:
    def __init__(self, file_v):
        self.extract_list = []  # exclude comment lines
        self.port_list = []  # name, io, width, signed, comment
    ...
    def extract_module_name():
        ...
        (\w+)       # 2 module name
        ''', re.VERBOSE)
        return regex_module_head

    def extract_module_ports():
        ...
        for unit in self.extract_list:
            ...
            if re_ports_obj is not None:
                ...
                port_info = {'name': port_name,
                             'direct': port_direction, 'width': port_width}
    ...
    def gen_module_instance(self, filename, mode):
        ...
        for unit in self.info_list:
            if index == ports_num - 1:
                post_fix = '//{:<15}width:{}'.format(
                    unit['direct'], unit['width'])
            else:
                post_fix = ',//{:<15}width:{}'.format(
                    unit['direct'], unit['width'])
            index += 1
            ...
            if mode == 'gen_inst_wc':
                line_list.append('.{:<30}{:<30}{}'.format(
                    unit['name'], '('+unit['name']+')', post_fix))
            elif mode == 'gen_inst_only':
                line_list.append('.{:<30}{:<5}{}'.format(
                    unit['name'], '(' + ')', post_fix))
        line_list.append(');')
